...
/// 但曲线也不能太陡——三次方在 v=0.3 就只剩 2.7%（约 −31dB），滑块前三分之一整段听不见，
/// 1.5 次方（0.3 → 16%）实测仍偏小。这里取 1.2 次方，只在纯线性上加一点弯：
/// v=0.1 → 6.3%、v=0.3 → 24%、v=0.5 → 44%、v=0.75 → 71%，滑块任何位置都有可用音量。
/// 两端精确：v=0 是真静音、v=1 是原始电平（满音量时乘 1.0，不引入任何量化误差）。
///
