...
/// streaming-speech 仓 server/compose.yaml),vLLM 是宿主进程。bind 由调用方传入
/// (install 时 `--bind` 写进 unit 的 Environment=ORCH_BIND,serve 经 clap default/env 读)。
fn cfg(bind: String) -> Cfg {
    Cfg {
        bind,
        ...
        vllm_base: std::env::var("VLLM_BASE")
            .unwrap_or_else(|_| "http://127.0.0.1:8085/v1".into()),
...
/// 起 daemon:打开 workspace 下的 app.db、播种运行时配置、挂保留清理任务、起 axum。
async fn serve(bind: String, workspace: PathBuf) -> anyhow::Result<()> {
    let c = cfg(bind);
