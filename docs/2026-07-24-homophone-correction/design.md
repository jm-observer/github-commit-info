...
> **术语（2026-07-25）**：本节的「P0 数据收集」= **场景记录**（`scene_log`），是与**纠错采集**
> （`correction_capture`）并列的独立功能。命名锚点与二者边界见
> [../2026-07-21-speech-correction-capture/design.md](../2026-07-21-speech-correction-capture/design.md) 的「术语锚点」节。
>
> **实现演进（2026-07-25）**：原设计把应用上下文塞进 `speech_samples` 的五列（只在纠错采集时才写，
> 天然只覆盖出错的样本、严重偏样）。已改为**独立表 `speech_scenes` + 每次交付自动记**（选项 A：
> 交付时记，App 必准，不开自动交付则不记）。`speech_samples` 的五列保留但仅纠错样本用，两条线彻底分开。
> 桌面端语音页有「场景记录」卡片显示实时读数（总数/今日/按应用排行/抓拍覆盖缺口），卡片右上角
> 是记录总开关（默认开，关掉只停落库）。
>
> **一行 = 一次交付**：`auto_paste` 合并链逐次增量打字时，worker 用累计全文覆盖同一行，不切成
> 半截话；`text` 因此始终是「实际交付出去的整段」。分析时可直接按行取用，不必再拼。
> 记录质量的四道闸（待交付内容保质期/内容校验、累计覆盖、回标时间窗+会话、session_id 落实）
> 见 [../2026-07-21-speech-correction-capture/design.md](../2026-07-21-speech-correction-capture/design.md)。
