...
>
> **2026-08-11 更新（v5）**：数据够了，读完了，**「按场景」这个前缀可以摘掉**——1481 条交付里
> 单一场景占 82%，不分组（§4.1）。同时数据表明**同音字错只占纠错样本的少数**，主导错误是专名/
> 术语没进热词表（§4.2）。**下一步是热词自举（§4.3），替换线（P3）继续挂起**——它卡在负样本量上，
> 不是卡在没实现。
...
| **v5**（2026-08-11 P1 读数） | **P1 结论落定：不分组**（§4.1，1481 条实测，单一场景占 82%），`app_profile` 列**不加**；**并附带一条推翻性发现**：真实纠错样本里同音字错只占少数，主导错误是「专名/术语未进热词表」（§4.2）→ **热词自举提为下一步，替换线（P3）继续挂起** |
...
## 4. 场景分层 —— P1 读数结论：**不分组**（2026-08-11 定案）
...
P0 攒够数据后读了一次，**数据的回答是「不需要场景词表」**。以下为定案依据，`app_profile`
列**不加**，§5 的 `scope` 字段保留但恒取 `default`（导出 JSON 里仍写 `app_exe` 原值，只作人工审读线索）。

### 4.1 读数：18 天 / 1481 条交付 / 21 个 exe

统计口径：`speech_scenes` 全表，2026-07-25 00:34 ~ 2026-08-11 22:50，累计 46426 字。

| 场景 | 条数 | 占比 | 备注 |
|---|---|---|---|
| `claude.exe` | 888 | 60.0% | |
| `ChatGPT.exe` | 222 | 15.0% | |
| `chrome.exe` | 104 | 7.0% | 标题几乎全是自家页面（网关入站请求 / 机群 429 汇总 / 账号列表…） |
| **以上三者合计** | **1214** | **82.0%** | **同一件事：跟 AI 讲技术** |
| `Weixin.exe` | 81 | 5.5% | 唯一用词分布可能不同的场景 |
| `rustrover64.exe` | 52 | 3.5% | |
| 其余 16 个 exe | 134 | 9.0% | 全是长尾，单个 ≤31 条（explorer / WindowsTerminal / 游戏 / LockApp…） |

三条推论：

1. **只有一个场景。** 占 82% 的三个 exe 内部用词分布是同一个（技术讨论），按 exe 切开等于把
   同一个分布切成三份，只会稀释每份的样本量，不会带来任何区分度。
2. **唯一有区分度的候选（微信）样本量不够。** 81 条撑不起一张独立词表，更撑不起独立评估
   （§9.4 的样本量门槛是按 600 条负样本算的，微信一年也攒不到）。
3. **长尾不可能有词表。** 16 个 exe 合计 134 条，单个最多 31 条，其中大半（explorer /
   LockApp / 游戏）根本不是「写文本」的场景。

因此 **`app_profile` 列不加、场景词表不建**，与 v4「分组由数据决定」一致——数据决定了不分组。
`app_title` 维度（原待办③）同样不必再做：chrome 的标题虽有区分度，但都指向同一批技术页面，
分不出第二个语域。

**复核触发条件**（满足任一再重开本节，否则不再讨论分组）：某个非 AI 对话场景累计 ≥300 条交付，
且其纠错样本里出现了与 AI 场景**冲突**的同音对（同一读音在两个场景要写成不同的字）。
注意是「冲突」不是「不同」——不冲突的词直接进全局表即可，不需要场景化。

### 4.2 附带发现：主导错误不是同音字，是热词缺失

读数时把 30 条纠错样本（29 条带 `correction`）逐条过了一遍读音等价筛子：

| 类别 | 条数 | 例 |
|---|---|---|
| **B 类·真同音（exact）** | **3** | 编义→编译、请求投→请求头、注流→驻留 |
| B 类·近音（fuzzy，不自动进候选） | ~4 | 结成→携程、算数据库→双数据库、赏臂→闪避、签名吃→签名池 |
| A 类·声学听错 | ~4 | 明火→零合、明映→灵魂 |
| C 类·事后改写 / 补内容 | ~11 | 补一句前缀、改数字、换说法 |
| 专名/术语替换（非纠错） | ~7 | Claude Code→mihomo、投研→同花顺、PE→C1 |

**产出率约每 500 条交付出 1 条 exact pair。** 照此速率，凑够 20 条 pair 需要约 1 万条交付、
大半年；而 §9.4 的上线门槛还额外要 ≥600 条负样本（当前为 **0**，且 1453 条「没被纠错」的
交付**不能**当负样本——无金标，等于默认「没改=正确」，见评审 1）。**P3 替换线继续挂起，
不是因为没做，是因为原理上还够不着。**

反过来，那批样本里反复出现的是**专名/术语没被认出来**：携程、驻留、签名池、zuche、cookie、
bin、wire guard、mihomo。7 条 `hotword` 样本的 `correction` 字段直接就是词条本身。这类错
**热词表能治**，且热词是「提示」不是「替换」，**没有过纠风险**（§0 的硬约束不适用）。

→ 下一步转向**热词自举**（§4.3），同音字替换线保持挂起。

### 4.3 热词自举（下一步，P1.5）

目标：把「用户说了但系统不认识的词」自动捞出来，人工勾选后一键进 `asr.hotwords`，
替代现在「只有手动打 `hotword` 标签才进一个词」的单点通路（§2.1）。

沿用 P2a 的形状与既定取舍：**纯函数挖掘 + 导出人工审读 + 不自动上线**（§10 取舍 1）。
两路输入：

1. **纠错样本侧**：对全部带 `correction` 的样本跑 diff，取 Y' 侧新增/替换出来的词面
   （不限 `hotword` 标签）——用户特地改出来的词，就是最该进热词的词。
2. **场景记录侧**（1481 条交付文本）：无分词依赖的新词发现——拉丁/混合 token 直接抽；
   中文候选走 n-gram 频次 + 左右邻字熵（边界稳定性），只留高频且边界稳的串。

产物与现有 `asr.hotwords` 比对去重后写 JSON。**不碰 `→` 语法**（§2.4）：热词条目只写词面，
不承载任何替换规则。

#### 4.3.1 实现（2026-08-11 落地）

| 文件 | 内容 |
|---|---|
| `.../speech/hotword_mine.rs` | 新增。纯函数层：`mine_corrections` / `mine_scenes` / `drop_existing` + 11 个单测 |
| `.../speech/homophone.rs` | `diff_blocks`/`Block`/`is_han`/`context_snippet` 提为 `pub(crate)`，`Block::Change` 加 `ins_start`（取 Y' 侧上下文）。**两处共用同一套 diff**，不写第二份对齐逻辑 |
| `.../speech/db/{repository,mod}.rs` | `list_scene_texts()`：读 `speech_scenes.text` |
| `.../speech/commands/samples.rs` | `speech_export_hotword_candidates` 命令；导出前 GET `/api/config` 拿现有热词去重 |
| `ui/.../SpeechPage.tsx` + `api/tauri-client.ts` | 「导出热词候选」按钮（与「导出同音候选」并排） |

**与同音挖掘的分工**（关键取舍）：热词侧跳过的是**同音挖掘实际认领的块**，不是「等长纯汉字」
这个形状。理由有实例：「登录助理→登录驻留」等长纯汉字，但读音不等价（理 lǐ / 留 liú），
homophone 会整块丢弃——按形状排除的话这个词两边都漏，按认领结果排除才接得住。

**降噪手段**（无分词、无词典，全靠统计）：n-gram 频次 ≥5 + **文档频次 ≥3**（必须出现在多次
不同交付里）+ 左右邻字熵 ≥1.2 + PMI ≥5 + 首尾虚词表 + 噪声词表 + 去嵌套 + 双字降权。
反方向去嵌套（「子串频次 ≥ 父串 2 倍即丢父串」）**试过并撤掉**：它能干掉「台服务器」，
但同时杀掉「请求头」（「请求」141 次 vs 「请求头」24 次），而后者正是要的词；量词/方位
粘连改由首尾字表挡。

**真实数据实测**（1481 条场景记录 + 29 条纠错样本）：

- 纠错侧 15 条，几乎全可用：`hello` / `mihomo` / `签名池` / `驻留` / `zuche` / `cookie` /
  `携程` / `同花顺` / `C1` / `bin`；噪声 4 条（内容改写与声学错的残留，一眼可排）。
- 场景侧 133 条，主列表前 20：`服务器` / `签名池` / `请求方式` / `图片验证` / `浏览器` /
  `同花顺` / `登录驻留` / `语音识别` / `订单请求` / `数据库` / `请求头` / `账号详情` /
  `Claude` / `Code`…噪声约 3/20。双字中文单独分组（`from_scenes_bigram`）——没有通用词
  词典兜底时双字里通用词最多（数据/文档/分析），混进主列表会把专名淹掉。

**仍是「导出 + 人工审读」**，没有做自动写入 `asr.hotwords`：审读后逐条加即可，
自动上线一旦灌进脏词会持续影响后续所有识别（§10 取舍 1 的同一条理由）。
...
| 阶段 | 内容 | 准入门槛 |
|---|---|---|
| **P0** | **数据收集（§1）** | 无。**已落地并持续运行**（`speech_scenes` + 场景记录卡片） |
| ~~P1~~ | ~~看数据定分组维度，加 `app_profile` 并回填~~ | **已完成，结论=不分组（§4.1），`app_profile` 不加。本阶段就此关闭** |
| P2a | `homophone.rs` 纯函数 + 候选导出 | **已落地**（2026-07-29，commit 00e5caa） |
| **P1.5** | **热词自举：候选挖掘 + 导出 + 一键同步（§4.3）** | **当前在做**。无新依赖、不碰交付文本、无过纠风险 |
...
7. **不做替换回滚**，只留审计表。
8. **分组维度由数据决定，不预设**（v4）→ **已执行，结论是不分组**（v5，§4.1）。
